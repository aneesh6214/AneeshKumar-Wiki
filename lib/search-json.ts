import { getAllJSONContent, extractSearchableText, extractSections, JSONContent } from './json-content'
...
export async function searchJSONContent(query: string): Promise<SearchResult[]> {
  if (!query.trim()) {
    return []
  }
...
export async function getJSONNavigationSuggestions(): Promise<SearchableContent[]> {
  const searchableContent = await getSearchableContent()
